...
class DataLoader:
    ...
    def load_data(self, x: List[str], y: List[str], dgs:bool = True, actuals: List[str] = None, adj_close: bool = True) -> None:
        """
        Load the bond data from parquet file.
        if running dgs values, just pass the values to column values as a list to the parameter x, all
        other parameters are ignored.
        x, y, and actual values are processed in the model training and pre-processing steps
    ...
    def get_var_window_data(
        self,
        start_idx: int,
        end_idx: int,
        preprocessor: 'VARDataPreprocessor'
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Extract VAR-formatted data for a specific window.

        Process:
        1. Extract raw DGS data for window
        2. Apply preprocessor transformations
        3. Return stacked lags (X) and targets (y, y_noise)

        Args:
            start_idx: Window start index
            end_idx: Window end index
            preprocessor: Fitted VARDataPreprocessor instance

        Returns:
            X: Stacked lag features (n_samples, n_lags * n_outputs)
            y: Smoothed targets (n_samples, n_outputs)
            y_noise: Unsmoothed actuals (n_samples, n_outputs)
        """
        if self.data is None:
            raise ValueError("Data must be loaded first")
        ...
        # Extract window data
        window_data = self.data.iloc[start_idx:end_idx]
        df_dgs = window_data[preprocessor.dgs_columns]

        # Apply preprocessing to window
        df_processed, X, y = preprocessor.preprocess_pipeline(df_dgs)

        # y_noise: use raw values shifted by forecast horizon
        # For horizon=1, we want the next day's actual values
        y_noise = df_dgs.shift(-1).iloc[:-1]  # Remove last row (no future value)

        # Align all arrays to the same length
        min_len = min(len(X), len(y), len(y_noise))

        return X[:min_len], y[:min_len], y_noise[:min_len].values

    def prepare_var_data(
        self,
        dgs_columns: List[str],
        ewm_span: int = 7,
        n_lags: int = 15,
        rounding_digits: int = 3
    ) -> 'VARDataPreprocessor':
        """
        Create and fit VAR preprocessor on loaded data.

        Args:
            dgs_columns: List of DGS column names
            ewm_span: Span for exponential weighted moving average
            n_lags: Number of lag features to create
            rounding_digits: Decimal places for rounding

        Returns:
            Fitted VARDataPreprocessor instance
        """
        if self.data is None:
            raise ValueError("Data must be loaded first")
        ...
        # Process full dataset to fit preprocessor
        df_dgs = self.data[dgs_columns]
        df_processed, X, y = preprocessor.preprocess_pipeline(df_dgs)

        logger.info(f"VAR preprocessor prepared: X shape={X.shape}, y shape={y.shape}")
